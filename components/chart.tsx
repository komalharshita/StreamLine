...
export function Chart({ title, type }: { title: string; type: 'line' | 'bar' }) {
  return (
    <div 
      className="bg-card/40 border border-border/80 rounded-xl p-5 space-y-5 select-none shadow-sm"
    >
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-semibold text-white uppercase tracking-wider">{title}</h3>
        <span className="text-[9px] uppercase tracking-wider font-bold text-muted-foreground/60 bg-card border border-white/5 px-2 py-0.5 rounded">Live Data</span>
      </div>
            ...
            {[65, 45, 78, 52, 88, 61, 73].map((height, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2 group cursor-pointer">
                ...
                <div className="w-full relative h-full flex items-end">
                  <div
                    className="w-full bg-muted hover:bg-secondary rounded-t-md transition-all duration-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] relative overflow-hidden"
                    style={{ height: `${height}%` }}
                  >
                    <div className="absolute inset-0 bg-secondary/15 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                </div>
                <span className="text-[10px] text-muted-foreground/70 group-hover:text-slate-200 font-semibold transition-colors">W{i + 1}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {/* Legend Footer */}
      <div className="flex items-center gap-4 pt-3.5 border-t border-border/40 text-[10px] font-semibold text-muted-foreground/70">
        <div className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-accent shadow-[0_0_6px_rgba(0,212,255,0.6)]"></span>
          <span className="text-slate-400">Primary Metric</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-muted border border-border/80"></span>
