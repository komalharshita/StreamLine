    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 select-none">
      ...
      <div className="flex justify-between items-center border-b border-border/40 pb-5">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">AI-generated business insights and operations ranked by priority and financial leverage.</p>
        </div>
        <button 
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
        ...
        <div className="bg-card border border-border/80 rounded-lg p-1 flex gap-1">
      ...
      <div className="space-y-3.5">
        {loading ? (
          ...
          Array.from({ length: 3 }).map((_, i) => (
            <div 
              key={i} 
              className="bg-card/40 border border-border/80 rounded-xl p-6 space-y-4 animate-pulse"
            >
          ...
          <div className="text-center py-12 bg-card/20 border border-rose-500/10 rounded-xl text-rose-400 space-y-4 max-w-xl mx-auto flex flex-col items-center justify-center p-6">
            <div className="w-10 h-10 rounded-lg bg-card border border-white/5 flex items-center justify-center text-rose-400">
              <AlertCircle className="w-5 h-5" />
            </div>
            <div className="space-y-1">
              <p className="font-semibold text-xs text-slate-200">Ingestion Inaccessible</p>
              <p className="text-[11px] text-muted-foreground/80 max-w-xs leading-normal">{error}</p>
            </div>
            <button 
              onClick={fetchDecisions}
          ...
          <div className="text-center py-16 bg-card/10 border border-dashed border-border rounded-xl flex flex-col items-center justify-center space-y-3">
            <AlertCircle className="w-6 h-6 text-muted-foreground/40" />
            <div className="space-y-1">
              <p className="text-xs font-semibold text-slate-300">No decisions found</p>
              <p className="text-[11px] text-muted-foreground/75 max-w-xs leading-relaxed">Ingest corporate datasets or adjust search filters to identify active business vectors.</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-border/40">
        <div className="bg-card/40 border border-border/80 rounded-xl p-4.5 space-y-1.5">
          <p className="text-muted-foreground/70 text-[10px] uppercase font-bold tracking-wider">Total Decisions</p>
          <p className="text-2xl font-bold tracking-tight text-white">{loading ? '-' : totalDecisions}</p>
        </div>
        <div className="bg-card/40 border border-border/80 rounded-xl p-4.5 space-y-1.5">
          <p className="text-muted-foreground/70 text-[10px] uppercase font-bold tracking-wider">Average Confidence</p>
          <p className="text-2xl font-bold tracking-tight text-white">{loading ? '-' : `${avgConfidence}%`}</p>
        </div>
        <div className="bg-card/40 border border-border/80 rounded-xl p-4.5 space-y-1.5">
          <p className="text-muted-foreground/70 text-[10px] uppercase font-bold tracking-wider">Total Potential Value</p>
