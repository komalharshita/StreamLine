...
class DatasetManager:
    ...
    def ensure_dataset(self, dataset_id: str, location: str = "US") -> bigquery.Dataset:
        ...
        try:
            ...
            logger.info(f"BigQuery dataset '{dataset_id}' not found. Initializing creation.")
            try:
                ...
                logger.warning(f"BigQuery dataset '{dataset_id}' was created concurrently by another process.")
                return self.client.get_dataset(dataset_ref)
            except Exception as e:
                logger.error(f"Failed to create BigQuery dataset '{dataset_id}': {str(e)}", exc_info=True)
                raise
