...
class TableManager:
    ...
    def ensure_table(
        ...
        try:
            ...
            logger.info(f"BigQuery table '{dataset_id}.{table_id}' verified (already exists).")
            return table
        except NotFound:
            logger.info(f"BigQuery table '{dataset_id}.{table_id}' not found. Initializing creation.")
            try:
                ...
                logger.warning(f"BigQuery table '{dataset_id}.{table_id}' was created concurrently by another process.")
                return self.client.get_table(table_ref)
            except Exception as e:
                logger.error(f"Failed to create BigQuery table '{dataset_id}.{table_id}': {str(e)}", exc_info=True)
                raise
