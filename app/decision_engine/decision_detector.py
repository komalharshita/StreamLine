...
class DecisionDetector:
    ...
    def detect_sales_decisions(cls, df: pd.DataFrame) -> list[Decision]:
        ...
        rev_col = cls._find_column(
            df, ["revenue", "price", "amount", "sales"], "revenue"
        )
                ...
                if mid_point > 0:
                    ...
                    if hist_rev > 0:
                        ...
                        if drop_pct >= DEFAULT_THRESHOLDS["revenue_drop_percentage"]:
                            ...
                            rec, roi = (
                                RecommendationEngine.generate_recommendation_and_roi(
                                    "Revenue Drop", evidence
                                )
                            )
            ...
            try:
                    ...
                    .agg(
                        {
                            rev_col: "sum",
                            margin_col: (
                                "mean" if margin_col in df.columns else lambda x: 0.35
                            ),  # default margin
                        }
                    )
                    .reset_index()
                ...
                for _, row in prod_stats.iterrows():
                    ...
                    if p_rev > 20000:
                        ...
                        rec, roi = RecommendationEngine.generate_recommendation_and_roi(
                            "High Performing Products", evidence
                        )
    ...
    def detect_inventory_decisions(cls, df: pd.DataFrame) -> list[Decision]:
        ...
        # Resolve headers
        prod_col = cls._find_column(df, ["product", "item", "sku"], "product_name")
        stock_col = cls._find_column(
            df, ["stock_level", "stock", "quantity", "qty"], "stock_level"
        )
        safety_col = cls._find_column(
            df, ["safety", "threshold", "minimum"], "safety_stock"
        )
        vel_col = cls._find_column(
            df, ["velocity", "sales_velocity", "demand"], "sales_velocity"
        )
        ...
        for _, row in df.iterrows():
            try:
                ...
                if stock < safety or (
                    velocity > 0
                    and (stock / velocity)
                    < DEFAULT_THRESHOLDS["inventory_shortage_weeks"]
                ):
                    evidence = {
                        "product_name": p_name,
                        "stock_level": stock,
                ...
                if (
                    velocity > 0
                    and (stock / velocity)
                    >= DEFAULT_THRESHOLDS["inventory_overstock_weeks"]
                ):
                    evidence = {
                        "product_name": p_name,
                        "stock_level": stock,
            ...
            try:
                ...
                for _, row in cat_expenses.iterrows():
                    ...
                    if c_amt > (overall_avg * 1.20):
                        evidence = {
                            "expense_category": c_name,
                            "category_spend": c_amt,
