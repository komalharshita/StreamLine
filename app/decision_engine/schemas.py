...
class Decision(BaseModel):
    ...
    priority_score: float = Field(
        ..., ge=0.0, le=100.0, description="Calculated priority score (0-100)"
    )
    priority_level: str = Field(
        ..., description="Qualitative priority tier: Critical, High, Medium, Low"
    )
    category: str = Field(
        ...,
        description="Type of detected issue (e.g., Revenue Drop, Inventory Shortage)",
    )
    ...
    evidence: dict[str, Any] = Field(
        default_factory=dict, description="Key data metrics supporting the finding"
    )
    financial_impact: float = Field(
        ..., description="Projected financial impact in dollars"
    )
    confidence_score: float = Field(
        ..., ge=0.0, le=100.0, description="Inferred confidence level percentage"
    )
    recommendation: str = Field(
        ..., description="Deterministic actionable recommendation"
    )
    expected_roi: float = Field(
        ..., description="Expected Return on Investment percentage (e.g. 15.5%)"
    )
    status: str = Field(
        "active", description="Decision lifecycle state: active, dismissed, resolved"
    )
...
class DecisionRefreshRequest(BaseModel):
    ...
    override_thresholds: Optional[dict[str, float]] = Field(
        None, description="Optional custom limits"
    )
