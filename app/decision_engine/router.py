...
def get_decision_feed(
    ...
    priority_level: Optional[str] = Query(
        None, description="Filter decisions by priority: Critical, High, Medium, Low"
    ),
):
    ...
    decisions = decision_service.get_feed(
        category=category, priority_level=priority_level
    )
...
@router.post(
    "/refresh", response_model=list[Decision], status_code=status.HTTP_201_CREATED
)
def refresh_decision_feed(request: DecisionRefreshRequest):
    ...
    logger.info(
        f"Handling POST request to refresh feed. Workspace: {request.workspace}"
    )
