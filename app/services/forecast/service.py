...
class ForecastServiceInterface(BaseService, ABC):
    ...
    @abstractmethod
    def generate_forecast(self, dataset_id: str, target_column: str, periods: int) -> dict[str, Any]:
...
class ForecastService(ForecastServiceInterface):
    ...
    def generate_forecast(self, dataset_id: str, target_column: str, periods: int) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("BigQuery fetch failed; using mock time-series data for forecast skeleton.")
            rows = [{"ds": f"2026-01-{i:02d}", target_column: float(100 + i * 5)} for i in range(1, 31)]

        # 2. Convert and compute using RAPIDS cuDF or CPU Pandas
        if HAS_RAPIDS:
            logger.info("RAPIDS GPU acceleration active. Processing with cuDF and cuML.")
            try:
                ...
                future_index = cudf.DataFrame({"index_feat": [float(len(df) + i) for i in range(periods)]})
                predictions = model.predict(future_index)
                
                predicted_values = predictions.to_arrow().to_pylist()
                logger.info("RAPIDS forecast execution completed successfully.")
            except Exception as e:
                logger.error(f"RAPIDS GPU computation failed: {str(e)}. Falling back to CPU.")
                predicted_values = self._cpu_fallback_forecast(rows, target_column, periods)
        else:
            logger.info("RAPIDS GPU unavailable. Falling back to CPU (Pandas + Scikit-Learn).")
            predicted_values = self._cpu_fallback_forecast(rows, target_column, periods)

        return {
            "accelerator_used": "GPU (RAPIDS)" if (HAS_RAPIDS and "df" in locals()) else "CPU",
    ...
    def _cpu_fallback_forecast(
        ...
        future_index = pd.DataFrame({"index_feat": [float(len(df) + i) for i in range(periods)]})
        predictions = model.predict(future_index)
