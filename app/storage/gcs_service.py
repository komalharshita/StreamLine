...
class GCSService:
    ...
    def _get_client(self) -> storage.Client:
        ...
        try:
            ...
            logger.critical(
                f"Failed to connect to Google Cloud Storage client: {str(e)}",
                exc_info=True,
            )
    ...
    def upload_bytes(
        ...
        logger.info(
            f"Initiating upload of blob: '{blob_name}' to GCS bucket: '{target_bucket}'"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully uploaded blob to GCS. Location URL: {public_url}"
            )
            return public_url

        except GoogleAPIError as e:
            logger.error(
                f"GCS API Error during upload of blob '{blob_name}': {str(e)}",
                exc_info=True,
            )
            ...
            logger.error(
                f"Unexpected error during GCS upload of blob '{blob_name}': {str(e)}",
                exc_info=True,
            )
            raise StorageUploadError(
    ...
    def delete_blob(self, blob_name: str, bucket_name: Optional[str] = None) -> None:
        ...
        target_bucket = bucket_name or self.config.bucket_name
        logger.info(
            f"Initiating deletion of blob: '{blob_name}' from GCS bucket: '{target_bucket}'"
        )
        ...
        try:
            ...
            if blob.exists(timeout=GCS_TIMEOUT_SECONDS):
                ...
                logger.warning(
                    f"GCS delete notice: Blob '{blob_name}' does not exist in bucket."
                )

        except GoogleAPIError as e:
            logger.error(
                f"GCS API Error during deletion of blob '{blob_name}': {str(e)}",
                exc_info=True,
            )
            ...
            logger.error(
                f"Unexpected error during GCS deletion of blob '{blob_name}': {str(e)}",
                exc_info=True,
            )
            raise StorageDeleteError(
